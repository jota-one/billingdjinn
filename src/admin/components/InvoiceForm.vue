<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="card bg-base-200 p-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="form-control sm:col-span-2">
          <label class="label"><span class="label-text font-semibold">Client <span class="text-error">*</span></span></label>
          <Select
            v-model="form.client"
        ...
        <div class="form-control">
          <label class="label"><span class="label-text font-semibold">N° de facture <span class="text-error">*</span></span></label>
          <InputText v-model="form.invoice_number" placeholder="2026-001" required :disabled="readonly" class="w-full" />
        </div>
        ...
        <!-- Date -->
        <div class="form-control">
          <label class="label"><span class="label-text font-semibold">Date <span class="text-error">*</span></span></label>
          <input v-model="form.date" type="date" class="input input-bordered w-full" required :disabled="readonly" />
        </div>

        <!-- Date d'échéance -->
        <div class="form-control">
          <label class="label"><span class="label-text font-semibold">Date d'échéance</span></label>
          <input v-model="form.due_date" type="date" class="input input-bordered w-full" :disabled="readonly" />
        </div>
        ...
        <table class="table table-sm w-full">
          ...
          <tbody>
            <tr v-for="(line, index) in lines" :key="index">
              ...
              <td>
                <button v-if="!readonly" type="button" class="btn btn-xs btn-ghost" @click="removeLine(index)">
                  <span class="i-fa-solid-trash text-error"></span>
                </button>
              </td>
            </tr>
            <tr v-if="lines.length === 0">
              <td colspan="5" class="text-center text-base-content/40 py-4">Aucune ligne. Ajoutez une prestation.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="flex flex-col gap-4 max-w-sm ml-auto">
        ...
        <div class="flex flex-col gap-3">
          <label class="label cursor-pointer justify-start gap-3">
            <input type="checkbox" v-model="form.tva_enabled" class="toggle toggle-primary toggle-sm" :disabled="readonly" />
    ...
    <div class="card bg-base-200 p-6">
      ...
      <Textarea v-model="form.notes" placeholder="Conditions particulières, remarques..." :rows="3" :disabled="readonly" class="w-full" />
    </div>
